...
This script is used to properly load the data from the dataset 
for the U-Net encoder-decoder network architecture. The input 
of the network is a concatenation of the RGB image with different 
optical flow configurations. A K-Fold cross validation is applied 
with K=5 with a 80/20 split. 
...
def prepare_batch_images(imgs, cat_type):
	imgs = np.stack(imgs)
	...
	for i in range(len(reshape_imgs)):
		flow = estimate(torch.from_numpy(reshape_imgs[i][0]), torch.from_numpy(reshape_imgs[i][1])).detach().numpy()
		flow_reshape = np.transpose(flow, (1, 2, 0))

		u 	= flow_reshape[:,:,0]
		v 	= flow_reshape[:,:,1]
		mag = np.sqrt(u*u + v*v)

		u 	= cv.normalize(u, None, 0, 1, cv.NORM_MINMAX, dtype=cv.CV_32F)
		v	= cv.normalize(v, None, 0, 1, cv.NORM_MINMAX, dtype=cv.CV_32F)
		mag = cv.normalize(mag, None, 0, 1, cv.NORM_MINMAX, dtype=cv.CV_32F)

		flow_color = flow_to_image(flow_reshape).transpose(2, 0, 1) / 255.

		if cat_type == 'M':
			rgb_cat = np.expand_dims(np.concatenate((np.expand_dims(mag, axis=0), reshape_imgs[i][-1]), axis=0), axis=0)
		elif cat_type == 'UV':
			rgb_cat = np.expand_dims(np.concatenate((np.expand_dims(u, axis=0), np.expand_dims(v, axis=0), reshape_imgs[i][-1]), axis=0), axis=0)
		elif cat_type == 'UVM':
			rgb_cat = np.expand_dims(np.concatenate((np.expand_dims(u, axis=0), np.expand_dims(v, axis=0), np.expand_dims(mag, axis=0), reshape_imgs[i][-1]), axis=0), axis=0)
...
class MAVDataset(data.Dataset):
	...
	def custom_collate(self, batch):
		...
		return prepare_batch_images(imgs, self.cat_type), prepare_batch_targets(mask, self.cat_type), names
