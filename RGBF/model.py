...
The input of the network is a RGB image concatenated with different
optical flow configurations.  The result is a 4D tensor with the shape 
(N, C, H, W). For example RGBM means RGB image concatenated with the 
magnitude of the optical flow resulting in a tensor of (N, 4, H, W).
...
class Encoder1(nn.Module):
	def __init__(self, factor, sequence_size=2, in_channels=3):
...
class Decoder1(nn.Module):
	def __init__(self, factor, sequence_size=2):
...
class UNetFlow(nn.Module):
	def __init__(self, factor, sequence_size, in_channels):
