...
This script is used to train the 2-stream U-Net Encoder Decoder network
architecture. 1 network for optical flow and 1 network for RGB image. 
Both networks are fused together at the deepest layer of the encoder.  

"""

from model import UNetFlow_2Stream
...
for k in [1]:
	...
	model 		= UNetFlow_2Stream(2, sequence_size, in_channels=len(cat_type)).to(device)
		...
		for batch_i, (imgs, targets, _) in enumerate(dataloader):
			...
			for i in range(len(targets)):
				if torch.sum(targets[i]) != 0:
					TP = (np.logical_and(torch.round(x[i]).cpu() == 1, torch.round(targets[i]).cpu() == 1)).sum().numpy()
					TN = (np.logical_and(torch.round(x[i]).cpu() == 0, torch.round(targets[i]).cpu() == 0)).sum().numpy()
					FP = (np.logical_and(torch.round(x[i]).cpu() == 1, torch.round(targets[i]).cpu() == 0)).sum().numpy()
					FN = (np.logical_and(torch.round(x[i]).cpu() == 0, torch.round(targets[i]).cpu() == 1)).sum().numpy()
