...
the two-streamU-Net encoder-decoder network architecture 
...
for ds in dataset_folders:
	...
	dummy_input_1	= torch.randn(1, len(cat_type), resolution[1], resolution[0], dtype=torch.float).to(device)
	dummy_input_2	= torch.randn(1, 3, resolution[1], resolution[0], dtype=torch.float).to(device)

	# Warm Up GPU
	for _ in range(10):
		_ = model(dummy_input_1, dummy_input_2).to(device, dtype=torch.float)
