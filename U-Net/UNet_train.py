...
The network can include multiple frames by means of concatenation 
of the input channels. For example sequence_size = 2,means 2 concatenated 
RGB images as input to the network resulting in a 4D tensor with 
6 channels (N, 6, H, W).
...
for k in [1]:
	...
	model       = UNet(2, sequence_size).to(device)
