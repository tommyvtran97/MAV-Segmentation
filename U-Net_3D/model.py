...
Implementation of the 3D U-Net encoder-decoder network architecture. 
The input of the network is stack of RGB images 5D tensor
with the shape (N, C, D, H, W), where 'D' is the number of
stacked images. 
...
class Decoder1(nn.Module):
	def __init__(self, factor, sequence_size=1):
		...
		self.up4 = up_single(int(64/factor), int(64/factor), stride=(sequence_size, 1, 1))
...
class UNet_3D(nn.Module):
	def __init__(self, factor, sequence_size):
		...
		self.decoder1 	= Decoder1(factor, sequence_size)
		self.factor 	= factor
