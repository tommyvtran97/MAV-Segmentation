...
from modelLSTM import UNet_LSTM
...
# Load the model
gatenet 		= 'CyberZoo/CZ_LSTM_S20_B5_K1'
load_model 		= 'models/' + gatenet + '/'
model 			= UNet_LSTM(2, sequence_size).to(device)
	...
	with torch.no_grad():
		for batch_i, (imgs, targets, files) in enumerate(dataloader):
			...
			if save_image and sequence_size == 1:
				cv.imwrite(path_output + folder_class + files[0][-1].split('/')[-1], image)
			else:
