...
This script is used to properly load the data from the different
KITTI MOTS dataset for the U-Net encoder-decoder network architecture
with a recurrent module. A K-Fold cross validation is applied with K=5
with a 80/20 split. 
...
def prepare_dataset(path_to_data, batch_size, sequence_size, validate, k_fold):
	...
	for i in range(len(data_sequence)):
		...
		if (validate and validation) or (train and training):
			...
			# Temporal overlap of the image
			count_batch = 0
			
			for j in range(0, len(images), sequence_size):

				if validation:
					if count_batch == 4:
						count_batch = 0

					if count_batch == 0 and (j + (sequence_size*4)) > (len(images)):
						break

				if (j + (sequence_size - 1)) > (len(images) - 1):
					break

				if training and (seq_ctr == len(data_sequence) - len(k_subset)) and (batch_ctr % 4 == 0):
					if (j + sequence_size*4) > len(images):
						break
