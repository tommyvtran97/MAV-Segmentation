...
class Recurrent(nn.Module):
	def __init__(self, factor, in_channels, sequence_size):
		...
		self.convrnn = ConvRNN(input_dim=int(in_channels/factor), 
	...
	def forward(self, x, reset=False, reset_avg=False, detach=False):
		...
		out, self.states = self.convrnn(x, self.states)
...
class UNet_RNN(nn.Module):
	def __init__(self, factor, sequence_size=1):
		...
		self.RNN   		= Recurrent(factor, int(512), self.sequence_size)
		self.decoder1 	= Decoder1(factor)
		self.factor 	= factor

	def forward(self, x, sequence=None, reset=False, reset_avg=False, detach=False):
		x 		= self.encoder1(x)
		x[4] 	= self.RNN(x[4], reset=reset, reset_avg=reset_avg, detach=detach)
		x 		= self.decoder1(x)
		x 		= torch.sigmoid(x)
